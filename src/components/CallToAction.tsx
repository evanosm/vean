...
export default function CallToAction({ text, link, hasIcon = true }: { text: string, link?: string, hasIcon?: boolean }): JSX.Element {
    if (link) {
        return <Link href={link} className="flex flex-row gap-2 items-center text-light bg-dark text-md font-bold px-3 py-2 text-xs md:text-sm lg:text-lg hover:scale-105 transition-all duration-300">
                {text}
